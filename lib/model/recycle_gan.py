...
class ReCycleGAN(nn.Module):
    ...
    def updateGenerator(self, true_x_tuple, fake_y_tuple, true_x_next, fake_pred, P_X, P_Y, net_G, net_D):
        """
            ...
            Arg:    true_x_tuple    - The tuple tensor object in X domain, rank is [B, (t-1)*C, H, W]
                    ...
                    fake_pred       - The fake prediction of future frame
        ...
        self.loss_G += (self.criterion_adv(fake_pred, True) + 
            100 * self.criterion_l2(fake_x_next, true_x_next) + 
    ...
    def backward(self):
        ...
        for i in range(self.t, self.T):
            fake_b_pred = self.updateDiscriminator(fake_b_frame_list[i], true_b_frame_list[i], self.D_B)
            fake_a_pred = self.updateDiscriminator(fake_a_frame_list[i], true_a_frame_list[i], self.D_A)
            true_a_tuple = torch.cat(true_a_frame_list[i - self.t: i], dim = 1)
            true_b_tuple = torch.cat(true_b_frame_list[i - self.t: i], dim = 1)
            fake_a_tuple = torch.cat(fake_a_frame_list[i - self.t: i], dim = 1)
            fake_b_tuple = torch.cat(fake_b_frame_list[i - self.t: i], dim = 1)
            self.updateGenerator(true_a_tuple, fake_b_tuple, true_a_frame_list[i], fake_b_pred, self.P_A, self.P_B, self.G_B_to_A, self.D_B)
            self.updateGenerator(true_b_tuple, fake_a_tuple, true_b_frame_list[i], fake_a_pred, self.P_B, self.P_A, self.G_A_to_B, self.D_A)           
